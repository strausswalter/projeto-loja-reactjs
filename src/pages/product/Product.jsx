    ...
    <Grid container spacing={2} className="product">
      <Grid item xs={12} sm={5} className="imageSide">
        <Stack className="imageSide__image">
          <div
            className="imageSide__image-item"
            style={{
      ...
      <Grid item xs={12} sm={7} className="productItem">
        ...
        <div className="productItem__categories">
          {products[params.id].categories.map((categoryId) => (
            <Chip
              className="productItem__categories-label"
              label={categories[categoryId].name}
            />
          ))}
        </div>

        {products[params.id].promo_price ? (
          <Typography
            variant="p"
            component="p"
            className="productItem__promo_price"
          >
            {products[params.id].promo_price.toLocaleString("pt-br", {
              style: "currency",
              currency: "BRL",
            })}
          </Typography>
        ...
        <Typography
          ...
          style={{
            ...
            textDecoration: products[params.id].promo_price
              ? "line-through"
              : "none",
          }}
        >
          {products[params.id].price.toLocaleString("pt-br", {
            style: "currency",
            currency: "BRL",
          })}
        </Typography>
        ...
        <Typography
          variant="p"
          component="p"
          className="productItem__description"
        >
